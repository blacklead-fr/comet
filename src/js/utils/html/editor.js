import { panel as getPanel, frame as getFrame } from '../../editor/components/stored.js';
import { isFunction, isString, isNode, isBool, isObject } from '../is.js';
import { encodeChars, stripTags } from '../fill.js';
import { parseDataset, parseId } from '../parse.js';
 ...
 */
 export default function( source, options ){
 	...
 	const __core = {
 		...
 		element: {
 			...
 			build_meta: function(){
 				...
 				__core.element.id = parseId( options.id );
 				__core.element.slug = isString( options.slug ) ? options.slug.trim() : false;
 				__core.element.data = ( data = DATA.get( __core.element.id, 'elements' ) ) ? data : false;
 		...
 		get: {
 			...
 			textarea: function(){
 				...
 				for( t = 0; t < textarea.length; t++ ){
 					...
 					if( isString( textarea[t].name ) && textarea[t].name.trim() === meta_element.slug ){
 						return textarea[t];
 		...
 		toolbar: {
 			...
 			get: function(){
 				const meta = Global().get( __core.toolbar.slug );

 				if( !isObject( meta ) || !isNode( meta.node ) || meta.buttons.length < 1 ){
 			...
 			buttons: function(){
 				...
 				const onbutton = {
 					...
 					default: function( command, val ){
 						...
 						val = isString( val ) && ( val = val.trim() ).length > 0 ? val : null;
...

 				{
 					...
 					render: function( button ){
 						...
 						node( createLink ).on( 'click', function( ev ){
 							const val = isString( input.value ) ? ( stripTags( input.value ) ).trim() : '';
 			...
 			create: function(){
 				...
 				return Global().set( __core.toolbar.slug, meta, true );
 			...
 			set_position: function( data ){
 				...
 				if( !isObject( data ) || !( 'toolbar' in data ) || !( 'document' in data ) || !( 'frame' in data ) ){
 					return false;
 		...
 		editor: {
 			...
 			create: function(){
 				...
 				const tag = isString( options.tag ) ? ( options.tag.toLowerCase() ).trim() : 'p';
 				const oObject = _d.createElement( tags.indexOf( tag ) < 0 ? 'p' : tag );

 				oObject.className = 'comet-editable comet-ui';
 				oObject.innerHTML = isString( options.content ) ? stripTags( options.content, __core.get.tags( false ) ) : 'Write something here...';
 			...
 			focus: function( ev ){
 				...
 				if( !meta || !isNode( ev.target ) || ( buttons = meta.buttons ).length < 1 ){
 					return;
 				...
 				function isCommand( button, command ){
 					const cmd = parseDataset( button, 'command' );

 					return ( isString( cmd ) && cmd.trim() === command );
 				...
 				function parent( comp ){
 					...
 					for( b = 0; b < buttons.length; b++ ){
 						...
 						if( !isNode( buttons[b] ) || !isCommand( buttons[b], command ) ){
 							continue;

 						}
 						node( buttons[b] ).addClass( __core.classes.active );

 					}
 					parent( comp.parentNode );

 				}
 				nodes( buttons ).removeClass( __core.classes.active );
 				parent( ev.target );

 			},

 			change: function( ev, ui ){
 				const sanitized_content = encodeChars( stripTags( ui.innerHTML, __core.get.tags( false ) ) );
 				const data = {};
 				var textarea;

 				data[meta_element.slug] = sanitized_content;
 				DATA.set( meta_element.id, 'elements', data );
 	...
 	if( !isNode( source ) || source.parentNode === null || !meta_element.id || !meta_element.slug || !meta_element.data ){
 		return false;
