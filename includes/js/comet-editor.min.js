...
a[e];if(h.hasOwnProperty("name")&&("items"===e||h.hasOwnProperty("sections")&&"object"===typeof h.sections)&&("items"!==e||h.hasOwnProperty("tabs")&&"object"===typeof h.tabs)){var l="comet-modalTab"+e;var w="items"===e?h.tabs:h.sections;d+=f.generateTabMenu(l,h.name);h=f.buildTab(w,b,e);c+=f.generateTabContent(l,h)}}g.tabs=d;g.content=c;return g},buildTab:function(a,b){var e;if("object"!==typeof a)return"";var d="";if(arguments&&arguments[2]&&"items"===arguments[2]){d+='<div class="comet-editorModalItems"><ul class="comet-edModalItems">';
...
typeof b.type)return"";var d=b.type.trim();var g="comet-field comet-rendField";var c="comet-modalField"+a;var h="comet-modalControl comet-modalControl"+cometUtils.generate.capitalize(d);var l="true"===b.responsive?!0:!1;var w='data-type="'+d+'"';"color"===d&&(g+=" comet-fieldColor");"true"==b["switch"]&&(g+=" comet-fieldSwitch");var p=f.getValue(a,b,e);h='<div class="'+h+'">';switch(d){case "editor":case "textarea":if("editor"===d&&(g+=" comet-fieldEditor","option"in b))switch(b.option){case "advanced":g+=
" comet-fieldEditorAdvanced";break;case "force_tag":g+=" comet-fieldEditorForceTag"}h+='<textarea id="'+c+'" name="'+a+'" class="'+g+'" '+w+">"+p+"</textarea>";break;case "range":var n="";"min"in b&&(n+=' min="'+b.min+'"');"max"in b&&(n+=' max="'+b.max+'"');"step"in b&&(n+=' step="'+b.step+'"');h=h+'<div class="comet-editRange"><button class="comet-eRDecrease comet-button">-</button><div class="comet-eRange"><div class="comet-eRLine"></div><div class="comet-eRDragger"></div></div><button class="comet-eRIncrease comet-button">+</button>'+
('<input type="hidden" name="'+a+'" class="'+g+'" value="'+p+'" '+n+" "+w+"/>")+('<div class="comet-eRValueUnit"><span class="comet-eRValue">'+p+"</span>"+("unit"in b?b.unit:"")+"</div>");h+="</div>";break;case "number":h+='<input type="number" id="'+c+'" name="'+a+'" class="'+g+'" value="'+p+'" '+w+"/>";"unit"in b&&(h+='<span class="comet-modalFieldUnit">'+b.unit+"</span>");break;case "numbers":if("object"!==typeof(d=b.values))return"";var k=function(a,c,d,b,g){var h="comet-modalField"+a,e="cpb-active";
"d"!==b&&(e="");a='<div class="comet-modalFieldWrap '+e+'" data-device="'+b+'">'+('<input id="'+h+'" type="number" name="'+a+'" class="'+g+'" value="'+c+'" '+w+"/>");return a=a+('<label for="'+h+'">'+d+"</label>")+"</div>"};h+='<button class="comet-button comet-edControlInline comet-editorLock" title="'+cometdata.ui.unlocked+'"><span class="cico cico-unlock"></span></button>';l&&(h=h+'<div class="comet-button comet-edControlInline comet-editorSwitchDevice"><span class="cico cico-desktop"></span><div class="comet-edSDevices">'+
...
c+'">',c="comet-modalFieldRadioLabel","icon"in d[n]&&(h+='<span class="comet-modalFieldRadioIcon '+d[n].icon+'"></span>',c+=" comet-tooltip"),h+='<span class="'+c+'">'+d[n].title+"</span>",h+='<input id="'+e+'" type="radio" name="'+a+'" class="'+g+'" value="'+n+'" '+l+" "+w+"/>",h+="</label>");h+="</div>";break;case "select":if("object"!==typeof(d=b.values))return"";"switch"in b&&"true"==b["switch"]&&"object"===typeof b.to&&(w+=' data-switch="'+encodeURI(JSON.stringify(b.to))+'"');h+='<select id="'+
c+'" name="'+a+'" class="'+g+'" '+w+">";for(n in d)l=n===p?'selected="selected"':"",h+='<option value="'+n+'" '+l+">"+d[n]+"</option>";h+="</select>";break;case "image":h=h+'<div class="comet-editorImage">'+f.kit.image(p);h=h+('<input type="hidden" id="'+c+'" name="'+a+'" class="'+g+'" value="'+p+'" '+w+"/>")+"</div>";break;case "icon":h+='<div class="comet-editorIcon">';h+=f.kit.icon(p);h=h+('<input type="hidden" id="'+c+'" name="'+a+'" class="'+g+'" value="'+p+'" '+w+"/>")+"</div>";break;case "gradient":(e=
...
l+'"/></div>');h+="</div>";h=h+('<input type="hidden" id="'+c+'" name="'+a+'" class="'+g+'" value="'+p+'" '+w+"/>")+"</div>";break;case "checkbox":h+='<input type="checkbox" id="'+c+'" name="'+a+'" class="'+g+'" value="true" '+w+" "+("true"==p?'checked="checked"':"")+"/>";break;default:"color"===d&&(h+='<button class="comet-button comet-modalControlClearColor">Clear</button>'),h+='<input type="text" id="'+c+'" name="'+a+'" class="'+g+'" value="'+p+'" '+w+"/>"}return h+="</div>"},getValue:function(a,
...
typeof t&&t.hasOwnProperty("std")?w[x]=t.std:w[x]=m}else w[g]=m}}}return f.insertMetaData(a,w,b)},insertId:function(a,b,e,d){var g;a=parseInt(a);e=parseInt(e);if("number"!==typeof a||isNaN(a)||"object"!==typeof k||"object"!==typeof k[b]||!(a in k[b])||"number"!==typeof e||isNaN(e))return!1;switch(b){case "sections":var c=b;break;case "rows":c="sections";break;case "columns":c="rows";break;case "elements":c="columns";break;case "items":c="elements";break;default:return!1}var h="_"+b;if("sections"===
...
e);if("number"!==typeof p||isNaN(p))p=0;p>=b&&(p=b);0>=p&&(p=0);var k=Number((p/b*100).toFixed(2));a.style.left=k+"%";p=c*Math.round(p/(b||1)*(g-d)/c)+d;p=Number(p.toFixed(2));for(k=0;k<n.length;k++){var w=n[k];var z=m(w);if(z.hasClass("comet-field")){var u=w;z.val(p)}else z.hasClass("comet-eRValueUnit")&&z.children(".comet-eRValue").html(p)}u&&"object"===typeof u&&f.updateComponent(u)}}},start:{on:"mousedown",trigger:".comet-eRDragger","do":function(h,f){h.preventDefault();a=f;var l=a.parentNode;
var p=l.getBoundingClientRect();b=m(l).outerWidth();e=p.left;l=m(l.parentNode).children(".comet-field");d=parseInt(l.attr("min"))||0;g=parseInt(l.attr("max"))||360;c=parseInt(l.attr("step"))||1}},stop:{on:"mouseup",trigger:"html","do":function(h,f){h.preventDefault();e=b=a=null;d=0;c=g=1}},ui:{on:"click",trigger:".comet-eRDecrease, .comet-eRIncrease","do":function(a,c){a.preventDefault();var d=m(c),b=c.parentNode.children,h,g,e,l;if(b&&null!=b&&"object"===typeof b&&"length"in b&&!(1>b.length)){for(l=
0;l<b.length;l++){var p=b[l];var k=m(p);k.hasClass("comet-field")?e=p:k.hasClass("comet-eRValueUnit")?h=k.children(".comet-eRValue"):k.hasClass("comet-eRange")&&(g=p)}if("object"===typeof e&&"object"===typeof h&&"object"===typeof g){b=parseFloat(e.value);p=parseInt(e.getAttribute("min"))||0;k=parseInt(e.getAttribute("max"))||360;l=parseInt(e.getAttribute("step"))||1;if("number"!==typeof b||isNaN(b))b=p;d.hasClass("comet-eRDecrease")?b-=l:d.hasClass("comet-eRIncrease")&&(b+=l);b<p?b=p:b>k&&(b=k);b=
Number(b.toFixed(2));e.value=b;h.html(b);d=Number((b/(k-p)*100).toFixed(2));m(g).children(".comet-eRDragger").css({left:d+"%"});f.updateComponent(e)}}}}};for(h in l)(function(){var a=l[h];H.on(a.on,a.trigger,function(c){a["do"](c,this)})})(h)},gradient:function(){var a=null,b=null,e=null,d=null,g=!1,c,h;var l={move:{on:"mousemove",trigger:"html","do":function(c,d){if(null!==a&&null!==e&&null!==b){var h=parseInt(c.pageX-e);if("number"!==typeof h||isNaN(h))h=0;h>=b&&(h=b);0>=h&&(h=0);h=parseInt(h/b*
100);a.style.left=h+"%";f.updateGradient(a);f.updateComponent(a)}}},start:{on:"mousedown",trigger:".comet-eGColor","do":function(c,l){var f,p,k,w;h=setInterval(function(){m(".comet-fieldColor").spectrum("hide");a=l;f=l.parentNode;p=f.getBoundingClientRect();b=m(f).outerWidth();e=p.left;k=f.parentNode;w=m(k);d=w.children(".comet-eGDelete");g=!0;d.length&&!d.is(":visible")?d.show():d.length||(w.append('<button class="comet-eGDelete comet-button" title="'+cometdata.ui["delete"]+'"><span class="cico cico-trash"></span> '+
cometdata.ui["delete"]+"</button>"),d=w.children(".comet-eGDelete"))},500)}},stop:{on:"mouseup",trigger:"html","do":function(c,l){c.preventDefault();var f=m(c.target);clearInterval(h);g&&(null!==a&&f.hasClass("comet-eGDelete")&&(f=m(a.parentNode).children(".comet-eGColor"),2<f.length&&a.remove()),null!==d&&(d.remove(),d=null),e=b=a=null,g=!1)}},add:{on:"click",trigger:".comet-eGLine","do":function(a,c){a.preventDefault();var d=m(c.parentNode);if(d){var b=d.children(".comet-eGColor");if(!b||5>b.length)d.append('<div class="comet-eGColor" style="left:50%"><input class="comet-eGColorPicker comet-fieldColor" value="#FFF" type="text"/></div>'),
...
b.parentNode&&"object"===typeof b.parentNode&&b.parentNode.removeChild(b)},events:function(){var a=[],b;a[a.length]={on:"click",trigger:".comet-tempItemInsert","do":function(a,b){a.preventDefault();var c;"object"!==typeof b.dataset||"undefined"===typeof b.dataset.id||"number"!==typeof(c=parseInt(b.dataset.id))||isNaN(c)||cometUtils.load({id:c,"do":"meta"}).done(function(a){var c=!1,d,b,h,g,e,r;if("0"==a||"object"!==typeof(d=cometUtils.parse.json(a))||!("_sections"in d)||cometUtils.eval.empty(d._sections)||
"object"!==typeof d.sections)return!1;var q=cometUtils.parse.ids(d._sections,"array");if("object"!==typeof q||1>q.length)return!1;for(b=a=0;b<q.length;b++){var u=q[b];if("object"===typeof d.sections[u]){var A=f.generateId("sections");var v=m.extend(!0,{},d.sections[u]);v._rows="";v=f.insertDataTo({id:A,"in":"sections",pid:0,index:"last",data:v,to:k});"object"===typeof v&&(k=v,0===a&&(c=A,a++));if("_rows"in d.sections[u]&&"string"===typeof d.sections[u]._rows&&(u=cometUtils.parse.ids(d.sections[u]._rows,
"array"),!("object"!==typeof u||1>u.length)))for(h=0;h<u.length;h++){var D=u[h];if("object"===typeof d.rows[D]){var K=f.generateId("rows");v=m.extend(!0,{},d.rows[D]);v._columns="";v=f.insertDataTo({id:K,"in":"rows",pid:A,index:"last",data:v,to:k});"object"===typeof v&&(k=v);if("_columns"in d.rows[D]&&"string"===typeof d.rows[D]._columns&&(D=cometUtils.parse.ids(d.rows[D]._columns,"array"),!("object"!==typeof D||1>D.length)))for(g=0;g<D.length;g++){var E=D[g];if("object"===typeof d.columns[E]){var L=
f.generateId("columns");v=m.extend(!0,{},d.columns[E]);v._elements="";v=f.insertDataTo({id:L,"in":"columns",pid:K,index:"last",data:v,to:k});"object"===typeof v&&(k=v);if("_elements"in d.columns[E]&&"string"===typeof d.columns[E]._elements&&(E=cometUtils.parse.ids(d.columns[E]._elements,"array"),!("object"!==typeof E||1>E.length)))for(e=0;e<E.length;e++){var F=E[e];if("object"===typeof d.elements[F]){var B=f.generateId("elements");v=m.extend(!0,{},d.elements[F]);v._items="";v=f.insertDataTo({id:B,
"in":"elements",pid:L,index:"last",data:v,to:k});"object"===typeof v&&(k=v);if("_items"in d.elements[F]&&"string"===typeof d.elements[F]._items&&(F=cometUtils.parse.ids(d.elements[F]._items,"array"),!("object"!==typeof F||1>F.length)))for(r=0;r<F.length;r++)if(v=F[r],"object"===typeof d.items[v]){var C=f.generateId("items");v=m.extend(!0,{},d.items[v]);v=f.insertDataTo({id:C,"in":"items",pid:B,index:"last",data:v,to:k});"object"===typeof v&&(k=v)}}}}}}}}}c=cometUtils.layout(k).init(c);"object"===
...
cometdata.ui.tempname+'"/>');d+='<button class="comet-saveTempButton comet-button comet-buttonPrimary" title="'+cometdata.ui.save+'"><span class="cico cico-export"></span></button>';f.dialog.init({id:"comet-modalSaveTemplate",header:c,content:d+"</div></div>",done:{target:".comet-saveTempButton","do":function(a,c){var d=r.getElementById("comet-saveTempInput"),b;var g="";d&&null!==d&&"object"===typeof d||(g=cometdata.messages.notSaved+"<br>");if(!(k&&"_sections"in k)||1>k._sections.length)g=cometdata.messages.empty+
"<br>";if("string"!==typeof d.value||cometUtils.eval.empty(b=d.value.trim()))g+=cometdata.messages.noTitle;if(0<g.length)return d=r.createElement("div"),d.setAttribute("class","comet-saveTempErr"),d.innerHTML=g,c.parentNode.parentNode.appendChild(d),1;g={title:b,meta:k,content:f.get.content(),post_type:"comet_mytemplates"};cometUtils.load({"do":"save",data:JSON.stringify(g)})}}})}};a[a.length]={on:"click",trigger:"#comet-editorSaveButton","do":function(a,b){a.preventDefault();var c=parseInt(J.ID),
...
b.dataset.role)){c=g;if(f.node(g).hasClass("cpb-element")){var u=g;c=u.parentNode.parentNode}d=c.parentNode;var A=d.parentNode;g=A.parentNode;var v=g.parentNode;switch(e){case "section":if("object"!==typeof v.dataset||"undefined"===typeof v.dataset.id||"number"!==typeof(x=parseInt(v.dataset.id))||isNaN(x)||"object"!==typeof k.sections||!(x in k.sections))return;if("edit"===p){e=cometdata.options.section.edit;q.id=x;q.type="sections";q.node=v;c=f.buildTabs(cometdata.section,k.sections[x]);break}else if("del"===
p)f.removeMetaData(x,"sections"),v.parentNode.removeChild(v);else if("dup"===p){c=f.initCloneMetaData(x,"sections",0);u=cometUtils.layout(k).section(c);if(!("object"===typeof u&&"html"in u&&"css"in u))return;f.insert.html(C,u);f.insert.css(c,"sections",u)}return;case "row":if("object"!==typeof A.dataset||"undefined"===typeof A.dataset.id||"number"!==typeof(r=parseInt(A.dataset.id))||isNaN(r)||"object"!==typeof k.rows||!(r in k.rows))return;if("edit"===p){e=cometdata.options.row.edit;q.id=r;q.type=
"rows";q.node=A;c=f.buildTabs(cometdata.row,k.rows[r]);break}else if("del"===p)f.removeMetaData(r,"rows"),A.parentNode.removeChild(A);else if("dup"===p){if("object"!==typeof v.dataset||"undefined"===typeof v.dataset.id||"number"!==typeof(x=parseInt(v.dataset.id))||isNaN(x))return;c=f.initCloneMetaData(r,"rows",x);u=cometUtils.layout(k).row(c);if(!("object"===typeof u&&"html"in u&&"css"in u))return;f.insert.html(g,u);f.insert.css(c,"rows",u)}return;case "column":if("object"!==typeof c.dataset||"undefined"===
...
"columns",r);u=cometUtils.layout(k).column(c);if("object"!==typeof u)return;"html"in u&&f.insert.html(d,u);"css"in u&&f.insert.css(c,"columns",u);f.redefineColumns(d)}return;case "element":if("object"!==typeof u.dataset||"undefined"===typeof u.dataset.id||"number"!==typeof(n=parseInt(u.dataset.id))||isNaN(n)||"object"!==typeof k.elements||!(n in k.elements))return;if("edit"===p){if(!("_type"in k.elements[n])||"undefined"===typeof(z=k.elements[n]._type)||"object"!==typeof cometdata.elements[z]||"object"!==
typeof cometdata.elements[z].tabs)return;e=cometdata.options.element.edit;q.id=n;q.type="elements";q.node=u;c=f.buildTabs(cometdata.elements[z].tabs,k.elements[n]);break}else if("del"===p)f.removeMetaData(n,"elements"),m(u).remove();else if("dup"===p){if("object"!==typeof c.dataset||"undefined"===typeof c.dataset.id||"number"!==typeof(t=parseInt(c.dataset.id))||isNaN(t))return;n=f.initCloneMetaData(n,"elements",t);u=cometUtils.layout(k).element(n);if(!("object"===typeof u&&"html"in u&&"css"in u))return;
m(c).children(".cpb-columnContent").append(u.html);f.insert.css(n,"elements",u);cometUtils.init.icons();cometUtils.init.images()}return;default:return}"object"===typeof c&&"tabs"in c&&"content"in c&&(f.modal({id:!1,title:e,close:{inner:'<span class="cico cico-x"></span>',title:cometdata.ui.close,"do":function(a,c){q.id=null;q.node=null;q.state=null;q.type=null;f.toggle.editor("close");f.handleSize()}},content:c.content,tabs:c.tabs}),f.initModalParts())}}};a[a.length]={on:"input propertychange",trigger:".comet-field",
...
c+='<button class="'+t+'" data-role="'+e.trim()+'">'+m[e]+"</button>";c+="</div>";c+="</div>"}k.innerHTML=c;r.body.appendChild(k)}}};var e=function(a){var d=null,c=null,e=!1,f,k;a.handle=a.handle||"";a.connectWith=a.connectWith||"ul";a.items=a.items||"li";a.placeholder=a.placeholder||"cpb-placeholderUi";a.cursor=a.cursor||null;a.containment=a.containment||"body";a.bodyClass=a.bodyClass||"cpb-sorting";var p='<div class="'+a.placeholder+'"></div>';var n='<div id="comet-uiCursor" class="'+a.cursor+'"><span class="dashicons dashicons-move"></span></div>';
var t=m(a.containment);var q=function(a){"destroy"===a&&null!==c?(m(c).remove(),c=null):"object"===typeof a&&(null===c&&(m("body").append(n),c=r.getElementById("comet-uiCursor")),null!==c&&(a.preventDefault(),m(c).css({left:a.pageX+"px",top:a.pageY+"px"})))};var x=function(a){a=a.split(",");var c,b;if("object"!==typeof a||!a.length||1>a.length)return!1;var d=[];for(c=b=0;c<a.length;c++){var e=a[c];if(!("string"!==typeof e||1>e.length)){e=e.trim();var g=e.substr(0,1);switch(g){case ".":g="class";break;
case "#":g="id";break;default:continue}e=e.substr(1);d[b]={};d[b].type=g;d[b].str=e;b++}}return d};var z={move:{on:"mousemove",trigger:r,"do":function(b,e){var g;if(null!==c&&null!==d){var f=x(a.connectWith);var h=x(a.items);var l=function(){var c=b.pageY;if(!t||!t.length||1>t.length)if(t=m(a.containment),!t||!t.length||1>t.length)return;clearInterval(k);var d=t.scrollTop();var e=t.outerHeight();isNaN(d)||isNaN(e)||isNaN(c)||(0<d&&1>c?(t.scrollTop(d-1),k=setInterval(l,1)):c>e&&(t.scrollTop(d+1),k=
setInterval(l,1)))};l();q(b);if("object"===typeof f&&f.length&&!(1>f.length)){for(g=0;g<f.length;g++){var n=f[g];if("object"===typeof n&&n.hasOwnProperty("type")&&n.hasOwnProperty("str")){var u=n.type;n=n.str;if("class"===u&&m(b.target).hasClass(n)||"id"===u&&b.target.id===n)if(u=m(b.target),n=u.children(a.items),!("object"===typeof n&&0<n.length)){(h=r.getElementsByClassName(a.placeholder))&&h.length&&0<h.length&&m(h).remove();u.prepend(p);return}}}if("object"===typeof h&&h.length&&!(1>h.length))for(g=
0;g<h.length;g++)if(f=h[g],"object"===typeof f&&f.hasOwnProperty("type")&&f.hasOwnProperty("str")&&(u=f.type,n=f.str,"class"===u&&m(b.target).hasClass(n)||"id"===u&&b.target.id===n)){u=m(b.target);f=u.outerHeight();n=f/2;var w=b.pageY-u.offset().top;var y=u.outerWidth();var A=y/4;var z=b.pageX-u.offset().left;m("."+a.placeholder).remove();if(z<=A||w<=n&&z>=A&&z<=y-A){u.before(p);break}(z>=y-A||w<=f&&w>n)&&u.after(p)}}}}},start:{on:"mousedown",trigger:a.handle,"do":function(c,b){var g;c.preventDefault();
