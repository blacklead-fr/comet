...
b.style:{}}),a="data:application/json;charset=utf-8,"+encodeURIComponent(a),b=l.createElement("a"),b.setAttribute("href",a),b.setAttribute("class","comet-button comet-buttonPrimary"),b.setAttribute("download",encodeURI(e)+".json"),b.innerHTML=cometdata.ui.download,h='<div><span class="comet-dialogIconState dashicons dashicons-yes"></span></div>',h+='<div class="comet-message comet-messageSuccess">'+cometdata.messages.stNew,h+="</div>",g.innerHTML=h,g.appendChild(b)):(h='<div><span class="comet-dialogIconState dashicons dashicons-warning"></span></div>',
...
function(a,b){return function(a){a=JSON.parse(a.target.result);if(!("object"===typeof a&&"title"in a&&"object"===typeof a.meta&&"content"in a))return!1;a.post_type="comet_mytemplates";p.load({"do":"save",data:a}).done(function(a){m(l.getElementById(b)).children(".comet-waitWhileIcon").attr("class","dashicons dashicons-yes");a=m(l.getElementsByClassName("comet-importTemplateItem")).children(".comet-waitWhileIcon");if(!a||"object"!==typeof a||1>a.length)a=l.createElement("a"),a.href=encodeURI(cometdata.myTemplates),
a.setAttribute("class","comet-button comet-buttonPrimary"),a.innerHTML=cometdata.ui.done,e.appendChild(a)})}}(h,t);k.readAsText(h);g++}}e.innerHTML=f+"</ul>"}}}},preview:{open:function(){return{on:"click",trigger:".comet-templatePreview","do":function(a,d){a.preventDefault();var b;var c='<div id="comet-previewTemplateDialogContent">';"object"!==typeof d.dataset||"string"!==typeof d.dataset.id||"number"!==typeof(b=parseInt(d.dataset.id))||isNaN(b)?c=c+'<div><span class="comet-dialogIconState dashicons dashicons-warning"></span></div>'+
...
"string"!==typeof d.dataset.id||"number"!==typeof(b=parseInt(d.dataset.id))||isNaN(b)?(k.myTemplates["delete"].id=!1,k.myTemplates["delete"].context=!1):(k.myTemplates["delete"].id=b,k.myTemplates["delete"].context=d);b='<div id="comet-deleteTemplateDialogContent">';b=k.myTemplates["delete"].id?b+cometdata.messages["delete"]:b+cometdata.messages.notDel;b=b+'<div class="comet-buttonset text-right">'+('<button id="comet-doTemplateDelCan" class="comet-button comet-buttonCancel">'+cometdata.ui.cancel+
"</button>");k.myTemplates["delete"].id&&(b+='<button id="comet-doTemplateDelete" class="comet-button comet-buttonPrimary">'+cometdata.ui["delete"]+"</button>");k.dialog({id:"comet-deleteTemplateDialog",header:"<h4>"+cometdata.titles.delTemp+"</h4>",content:b+"</div></div>",done:{target:"#comet-doTemplateDelCan","do":function(a,b){a.preventDefault();return!0}}})}}},save:function(){return{on:"click",trigger:"#comet-doTemplateDelete","do":function(a,d){a.preventDefault();var b;var c=l.getElementById("comet-deleteTemplateDialogContent");
if(!c||null==c||"object"!==typeof c)return!1;var e='<div><span class="comet-dialogIconState comet-waitWhileIcon cico cico-spin"></span></div>'+cometdata.messages.wait;if(!k.myTemplates["delete"].id||"number"!==typeof(b=parseInt(k.myTemplates["delete"].id))||isNaN(b))e='<div><span class="comet-dialogIconState dashicons dashicons-warning"></span></div>'+('<div class="comet-message comet-messageError">'+cometdata.messages.notDel+"</div>");c.innerHTML=e;p.load({"do":"dtemplate",id:b}).done(function(a){var b=
l.getElementById("comet-deleteTemplateDialogContent");var c='<div><span class="comet-dialogIconState dashicons dashicons-yes"></span></div>'+('<div class="comet-message comet-messageSuccess">'+cometdata.messages.stDel+"</div>");"false"==a?c='<div><span class="comet-dialogIconState dashicons dashicons-warning"></span></div>'+('<div class="comet-message comet-messageError">'+cometdata.messages.notDel+"</div>"):k.myTemplates["delete"].context.parentNode.parentNode.remove();b.innerHTML=c})}}}},init:function(){var a=
